...
def calculate_demographic_data(print_data=True):
    ...
    # 2️⃣ Number of each race
    race_count = df['race'].value_counts()

    # 3️⃣ Average age of men
    average_age_men = round(df[df['sex'] == 'Male']['age'].mean(), 1)

    # 4️⃣ Percentage with Bachelor's degree
    percentage_bachelors = round((df['education'] == 'Bachelors').mean() * 100, 1)

    # 5️⃣ Percentage with higher education (>50K)
    higher_education = df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])
    higher_education_rich = round((df[higher_education]['salary'] == '>50K').mean() * 100, 1)

    # 6️⃣ Percentage without higher education (>50K)
    lower_education = ~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])
    lower_education_rich = round((df[lower_education]['salary'] == '>50K').mean() * 100, 1)

    # 7️⃣ Minimum work hours
    min_work_hours = df['hours-per-week'].min()

    # 8️⃣ Percentage rich among those who work minimum hours
    num_min_workers = df[df['hours-per-week'] == min_work_hours]
    rich_percentage = round((num_min_workers['salary'] == '>50K').mean() * 100, 1)

    # 9️⃣ Country with highest percentage of >50K
    ...
    # ✅ Return all results as a dictionary (very important)
    return {
